  ...
  >
    <v-card>
      ...
      <v-row class="my-2 mx-0 pa-0" justify="center" align="center">
        <v-btn :disabled="!valid" @click="addImage">
            ...
            <div :key="index" class="mt-3 pa-0">
              <v-row :id="`image_${index}`" class="ml-2 mr-3 pa-0">
                <v-col
                  align="center"
                  justify="center"
                  class="ma-0 mb-2 pa-0 elevation-1"
                  :cols="4"
                >
                  <v-img
...
export default {
  ...
  watch: {
    lightboxDialogState(val) {
      if (val) {
        const lightbox = this.formData.lightbox;
        if (lightbox) {
          const images = Array.isArray(lightbox)
            ? lightbox
            : JSON.parse(lightbox);
