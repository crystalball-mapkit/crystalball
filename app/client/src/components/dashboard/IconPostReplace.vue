    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-app-bar dark :color="color" dense flat>
          <v-app-bar-nav-icon
            ><v-icon>fas fa-exchange-alt</v-icon></v-app-bar-nav-icon
          >
          <v-toolbar-title class="white--text"
            >Update Existing Posts and Delete Icon</v-toolbar-title
          >
        </v-app-bar>
        <v-card-text>
          <v-container>
              ...
              <v-row>
                ...
                <v-col cols="12">
                  <v-select
                    ...
                    :items="
                      icons.filter(i => i.iconUrl !== icon.previousIconUrl)
                    "
                    item-text="iconUrl"
                    item-value="iconUrl"
                  ...
                  >
                    <template slot="selection" slot-scope="{ item }">
                      ...
                      <span class="short-text">{{
                        `Group: ${item.group} - Url: ${item.iconUrl}`
                      }}</span>
                    </template>
